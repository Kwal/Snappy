    ...
    internal static class Compressor
    {
        ...
        private static int EmitLiteral(byte[] output, int outputIndex, byte[] literal, int literalIndex, int length, bool allowFastPath)
        {
            ...
            else if (n < (1 << 16))
            {
                ...
                output[outputIndex++] = (byte)n;
                output[outputIndex++] = (byte)n.BitwiseUnsignedRightShift(8);
            }
            else if (n < (1 << 24))
            {
                ...
                output[outputIndex++] = (byte)n;
                output[outputIndex++] = (byte)n.BitwiseUnsignedRightShift(8);
                output[outputIndex++] = (byte)n.BitwiseUnsignedRightShift(16);
            }
            else
            {
                ...
                output[outputIndex++] = (byte)n.BitwiseUnsignedRightShift(8);
                output[outputIndex++] = (byte)n.BitwiseUnsignedRightShift(16);
                output[outputIndex++] = (byte)n.BitwiseUnsignedRightShift(24);
            }
        ...
        private static int EmitCopyLessThan64(byte[] output, int outputIndex, int offset, int length)
        {
            ...
            if ((length < 12) && (offset < 2048))
            {
                ...
                output[outputIndex++] = (byte)(COPY_1_BYTE_OFFSET | ((lenMinus4) << 2) | (offset.BitwiseUnsignedRightShift(8) << 5));
            ...
            else
            {
                ...
                output[outputIndex++] = (byte)offset;
                output[outputIndex++] = (byte)offset.BitwiseUnsignedRightShift(8);
            }
        ...
        private static int WriteUncompressedLength(byte[] compressed, int compressedOffset, int uncompressedLength)
        {
            ...
            else if (uncompressedLength < (1 << 14) && uncompressedLength > 0)
            {
                ...
                compressed[compressedOffset++] = (byte)uncompressedLength.BitwiseUnsignedRightShift(7);
            }
            else if (uncompressedLength < (1 << 21) && uncompressedLength > 0)
            {
                compressed[compressedOffset++] = (byte)(uncompressedLength | highBitMask);
                compressed[compressedOffset++] = (byte)(uncompressedLength.BitwiseUnsignedRightShift(7) | highBitMask);
                compressed[compressedOffset++] = (byte)uncompressedLength.BitwiseUnsignedRightShift(14);
            }
            else if (uncompressedLength < (1 << 28) && uncompressedLength > 0)
            {
                compressed[compressedOffset++] = (byte)(uncompressedLength | highBitMask);
                compressed[compressedOffset++] = (byte)(uncompressedLength.BitwiseUnsignedRightShift(7) | highBitMask);
                compressed[compressedOffset++] = (byte)(uncompressedLength.BitwiseUnsignedRightShift(14) | highBitMask);
                compressed[compressedOffset++] = (byte)uncompressedLength.BitwiseUnsignedRightShift(21);
            }
            else
            {
                compressed[compressedOffset++] = (byte)(uncompressedLength | highBitMask);
                compressed[compressedOffset++] = (byte)(uncompressedLength.BitwiseUnsignedRightShift(7) | highBitMask);
                compressed[compressedOffset++] = (byte)(uncompressedLength.BitwiseUnsignedRightShift(14) | highBitMask);
                compressed[compressedOffset++] = (byte)(uncompressedLength.BitwiseUnsignedRightShift(21) | highBitMask);
                compressed[compressedOffset++] = (byte)uncompressedLength.BitwiseUnsignedRightShift(28);
            }
        ...
        {
            return skip.BitwiseUnsignedRightShift(5);
        }

        private static int HashBytes(int bytes, int shift)
        {
            int multiplier = 0x1e35a7bd;
            return (bytes * multiplier).BitwiseUnsignedRightShift(shift);
        }
